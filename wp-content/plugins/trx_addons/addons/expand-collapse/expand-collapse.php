...
if ( ! function_exists( 'trx_addons_elm_add_params_expand_collapse' ) ) {
	...
	function trx_addons_elm_add_params_expand_collapse( $element, $section_id, $args ) {
		...
		if ( in_array( $element->get_name(), array( 'container', 'section', 'column', 'common', 'common-optimized' ) ) && $section_id == '_section_responsive' ) {
			...
			// Gradient settings
			$element->add_control( 'trx_expcol_gradient_heading', array(
			...
			$element->add_control( 'trx_expcol_gradient_color', array(
				...
				'selectors' => array(
					'{{WRAPPER}}:after' => 'background: -webkit-gradient(linear,left bottom,left top,color-stop(var(--trx-expcol-gradient-offset),{{VALUE}}),to({{VALUE}}00));'
										. 'background: -webkit-linear-gradient(bottom,{{VALUE}} var(--trx-expcol-gradient-offset),{{VALUE}}00 100%);'
										. 'background: linear-gradient(to top,{{VALUE}} var(--trx-expcol-gradient-offset),{{VALUE}}00 100%);',
			...
			$element->add_control( 'trx_expcol_gradient_offset', array(
				'label' => __( 'Gradient Offset', 'trx_addons' ),
				'type' => \Elementor\Controls_Manager::SLIDER,
				'default' => array(
					'size' => '0',
					'unit' => '%'
				...
				'selectors' => array(
					'{{WRAPPER}}.trx_expcol_gradient_on' => '--trx-expcol-gradient-offset: {{SIZE}}%;'
				),
			) );

			// Button settings
			$element->add_control( 'trx_expcol_button_heading', array(
			...
			if ( $styles_allowed ) {
				$element->add_group_control(
					...
					[
						'name' => 'trx_expcol_button_typography',
						'selector' => '{{WRAPPER}} .trx_expcol_button',
						'condition' => array(
							'trx_expcol_allow' => 'on',
						),
					]
			...
			$element->add_responsive_control( 'trx_expcol_button_offset', array(
				'label' => __( 'Button Offset', 'trx_addons' ),
				'type' => \Elementor\Controls_Manager::SLIDER,
				'default' => array(
					'size' => '',
					'unit' => 'px'
				...
				'selectors' => array(
					'{{WRAPPER}} .trx_expcol_button' => 'bottom: {{SIZE}}{{UNIT}};'
				),
			) );
